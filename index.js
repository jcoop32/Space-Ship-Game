...
console.log("  _..-''-------'   `'   `'   `'     O ``-''._   (,;') _,'");
console.log(",'________________                          \`-._`-','");
console.log(" `._              ```````````------...___   '-.._'-:");
console.log("    ```--.._      ,.                     ````--...__\-.");
console.log("            `.--. `-`                       ____    |  |`");
console.log("              `. `.                       ,'`````.  ;  ;`");
console.log("                `._`.        __________   `.      \'__/`");
console.log("                   `-:._____/______/___/____`.     \  `");
console.log('                               |       `._    `.    \"');
console.log("                               `._________`-.   `.   `.___");
console.log("                                                  `------'`");
...
console.log(
  `${alienShips.length} Alien space ships are threatening to destroy Earth.\n`
);
...
function fightTheShips() {
  ...
  if (USSS.hull < 15 && USSS.hull > 1) {
    let userRecharge = prompt(
      "Do you want to return to base and recharge shields? (y/n) "
    );
    console.log("\n");
    if (userRecharge === "y") {
      USSS.hull = 20;
...
function alienShipFires() {
  if (alienShips[alienShip].hull > 0) {
    ...
    if (firingSolution <= alienShips[alienShip].accuracy) {
      ...
      console.log(
        `You've taken ${alienShips[alienShip].firePower} damage and were protected by a shield strength of ${shield}.`
      );
    } else {
      //report out the miss
...
for (let ships of alienShips) {
  console.log(
    `You are now attacking ship # ${alienShip + 1} of ${alienShips.length}`
  );
  console.log("\n");
